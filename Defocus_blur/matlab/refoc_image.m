function [Im_refoc, Ad, sigma_vector, D,temp2]=refoc_image(im, depth, step_depth, focus, f, N, px, type_flou)
...
%         Para(k)=phi/2;
    ...
    if temp_phi > max_radius
        max_radius = temp_phi;
    end
    %disp(temp_phi);
    % mask extraction
    temp2(depth<D(k+1)&depth>=D(k))=temp_phi;%*2;
